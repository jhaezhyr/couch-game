...
function validateGameRoom(room: GameRoom) {
  ...
  if (emptySeats !== 1) {
    throw new Error(`Invalid game state: expected 1 empty seat, found ${emptySeats}`);
  }
}

export function makeMove(room: GameRoom, calledNameValue: string): { winner: string | null } {
  ...
  const allA = couchTeamIds.length > 0 && couchTeamIds.every(tid => tid === 'A');
  const allB = couchTeamIds.length > 0 && couchTeamIds.every(tid => tid === 'B');
