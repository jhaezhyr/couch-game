...
describe("WebSocket Integration Tests", () => {
  ...
  afterEach(() => {
    ...
    Object.keys(serverInstance.gameRoomSetups).forEach(
      (key) => delete serverInstance.gameRoomSetups[key]
    );
    Object.keys(serverInstance.gameRooms).forEach(
      (key) => delete serverInstance.gameRooms[key]
    );
  });
  ...
  it("should handle a complete move sequence", async () => {
    ...
    expect(
      gameRoom!.seats.filter((s: PlayerId | null) => s === null).length
    ).toBe(1); // Still one empty seat
